...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		return lib.notify({ id = 'inventory_right_access', type = 'error', position = "top", description = locale('inventory_right_access') })
	end
	...
	if canOpenInventory() then
		...
		if inv == 'shop' and invOpen == false then
			if cache.vehicle then
				return lib.notify({ id = 'cannot_perform', type = 'error', position = "top", description = locale('cannot_perform') })
			end
		...
		elseif invOpen ~= nil then
			if inv == 'policeevidence' then
				...
				if input then
					...
					return lib.notify({ position = "top", description = locale('locker_no_value'), type = 'error' })
				end

				if type(input) ~= 'number' then
					return lib.notify({ position = "top", description = locale('locker_must_number'), type = 'error' })
				else
					data = input
				end
			end
		...
		if left then
			...
			if invOpen == false then lib.notify({ id = 'inventory_right_access', type = 'error', position = "top", description = locale('inventory_right_access') }) end
			if invOpen then client.closeInventory() end
		end
	elseif invBusy then lib.notify({ id = 'inventory_player_access', type = 'error', position = "top", description = locale('inventory_player_access') }) end
end
...
lib.callback.register('ox_inventory:usingItem', function(data)
	...
	if item and invBusy then
		...
		local success = (not item.usetime or lib.progressBar({
			...
			useWhileDead = exports["visn_are"]:GetHealthBuffer().unconscious,
		...
		if success then
			if item.notification then
				lib.notify({ position = "top", description = item.notification })
			end
...
local function useSlot(slot)
	...
	if canUseItem(data.ammo and true) then
		if data.component and not currentWeapon then
			return lib.notify({ id = 'weapon_hand_required', type = 'error', position = "top", description = locale('weapon_hand_required') })
		end
		...
		if durability and durability <= 100 and consume then
			if durability <= 0 then
				return lib.notify({ type = 'error', position = "top", description = locale('no_durability', label) })
			elseif consume ~= 0 and consume < 1 and durability < consume * 100 then
				return lib.notify({ type = 'error', position = "top", description = locale('not_enough_durability', label) })
			end
		end
			...
			elseif data.component then
				...
				for componentIndex = 1, #weaponComponents do
					if componentType == Items[weaponComponents[componentIndex]].type then
						return lib.notify({ id = 'component_slot_occupied', type = 'error', position = "top", description = locale('component_slot_occupied', componentType) })
					end
				end
				...
				for i = 1, #components do
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							lib.notify({ id = 'component_has', type = 'error', position = "top", description = locale('component_has', label) })
						else
				...
				lib.notify({ id = 'component_invalid', type = 'error', position = "top", description = locale('component_invalid', label) })
...
local function registerCommands()
	...
	lib.addKeybind({
		name = 'inv',
		position = "top", description = locale('open_player_inventory'),
	...
	lib.addKeybind({
		name = 'inv2',
		position = "top", description = locale('open_secondary_inventory'),
		...
		onPressed = function()
			...
			if invBusy then
				return lib.notify({ id = 'inventory_player_access', type = 'error', position = "top", description = locale('inventory_player_access') })
			end
			...
			if GetVehicleDoorLockStatus(entity) > 1 then
				return lib.notify({ id = 'vehicle_locked', type = 'error', position = "top", description = locale('vehicle_locked') })
			end
	...
	lib.addKeybind({
		name = 'reloadweapon',
		position = "top", description = locale('reload_weapon'),
		defaultKey = 'r',
		onPressed = function(self)
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					lib.notify({ id = 'no_durability', type = 'error', position = "top", description = locale('no_durability', currentWeapon.label) })
				end
			end
		end
	})
	...
	lib.addKeybind({
		name = 'hotbar',
		position = "top", description = locale('disable_hotbar'),
	...
	for i = 1, 5 do
		lib.addKeybind({
			...
			position = "top", description = locale('use_hotbar', i),
...
RegisterNetEvent('ox_inventory:inventoryReturned', function(data)
	...
	lib.notify({ position = "top", description = locale('items_returned') })
	client.closeInventory()
...
RegisterNetEvent('ox_inventory:inventoryConfiscated', function(message)
	if source == '' then return end
	if message then lib.notify({ position = "top", description = locale('items_confiscated') }) end
	if currentWeapon then currentWeapon = Weapon.Disarm(currentWeapon) end
...
local function setStateBagHandler(stateId)
	...
	AddStateBagChangeHandler('dead', stateId, function(_, _, value)
		Utils.WeaponWheel()
		exports["visn_are"]:GetHealthBuffer().unconscious = value
	end)
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
				...
				lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
					if success ~= nil then
						lib.notify({
							...
							position = "top", description = locale(message, locale('license', point.type:gsub("^%l", string.upper)))
						})
					end
				end, point.invId)
			end
	...
	lib.notify({ position = "top", description = locale('inventory_setup') })
	Shops.refreshShops()
	Inventory.Stashes()
	Inventory.Evidence()
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 1.8 or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							client.closeInventory()
							lib.notify({ id = 'inventory_lost_access', type = 'error', position = "top", description = locale('inventory_lost_access') })
						else
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if data.slot ~= currentWeapon.slot then
		return lib.notify({ id = 'weapon_hand_wrong', type = 'error', position = "top", description = locale('weapon_hand_wrong') })
	end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	elseif response then
		if type(response) == 'table' then
			...
			lib.notify({ type = 'error', position = "top", description = locale(response) })
		end
	end
end)
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		...
		if not success then
			if response then lib.notify({ type = 'error', position = "top", description = locale(response or 'cannot_perform') }) end
			break
		end
	end
